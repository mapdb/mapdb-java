        ...
        implements ImmutableLongList, Serializable
{
    ...
    /**
     * Returns a new LongInterval with the same elements in the opposite order: it
     ...
     * minimum step".
     *
     * @throws ArithmeticException if the step is {@link Long#MIN_VALUE}
     */
    @Override
    public LongInterval toReversed()
    {
        ...
        // Computed with unsigned 64-bit remainder (never via size(), which is capped).
        long last = LongIntervalUtils.lastElement(this.from, this.to, this.step);
        return LongInterval.fromToBy(last, this.from, -this.step);
    }
