        ...
        implements ImmutableShortList, Serializable
{
    ...
    /**
     * Returns a new ShortInterval with the same elements in the opposite order: it
     ...
     * minimum step".
     *
     * @throws ArithmeticException if the step is {@link Short#MIN_VALUE}
     */
    @Override
    public ShortInterval toReversed()
    {
        ...
        short last = (short) IntervalUtils.lastElement(this.from, this.to, this.step);
        return ShortInterval.fromToBy(last, this.from, (short) -this.step);
    }
