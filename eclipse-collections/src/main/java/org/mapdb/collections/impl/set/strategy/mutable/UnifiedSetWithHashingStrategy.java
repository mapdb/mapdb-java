...
import org.mapdb.collections.impl.lazy.parallel.set.AbstractParallelUnsortedSetIterable;
import org.mapdb.collections.impl.lazy.parallel.set.RootUnsortedSetBatch;
import org.mapdb.collections.impl.lazy.parallel.set.SelectUnsortedSetBatch;
import org.mapdb.collections.impl.lazy.parallel.set.UnsortedSetBatch;
import org.mapdb.collections.impl.multimap.set.strategy.UnifiedSetWithHashingStrategyMultimap;
import org.mapdb.collections.impl.partition.set.strategy.PartitionUnifiedSetWithHashingStrategy;
import org.mapdb.collections.impl.set.AbstractUnifiedSet;
        ...
        implements Externalizable
{
    ...
    public static <K> UnifiedSetWithHashingStrategy<K> newSet(HashingStrategy<? super K> hashingStrategy, Iterable<? extends K> source)
    {
        ...
        UnifiedSetWithHashingStrategy<K> result = source instanceof RichIterable<?> ri
                ? UnifiedSetWithHashingStrategy.newSet(hashingStrategy, ri.size())
    ...
    public T addOrReplace(T key)
    {
        ...
        {
            return this.chainedAddOrReplace(key, bucket);
        }
    ...
    {
        if (iterable instanceof UnifiedSetWithHashingStrategy<?> unifiedSetWithHashingStrategy)
        {
            return this.copySet(unifiedSetWithHashingStrategy);
        }
    ...
    private final class UnifiedUnsortedSetBatch extends AbstractBatch<T> implements RootUnsortedSetBatch<T>
    {
        ...
        public void forEach(Procedure<? super T> procedure)
        {
            for (int i = this.chunkStartIndex; i < this.chunkEndIndex; i++)
            {
                ...
                if (cur instanceof ChainedBucket bucket)
                {
                    UnifiedSetWithHashingStrategy.this.chainedForEach(bucket, procedure);
                }
                else if (cur != null)
                {
